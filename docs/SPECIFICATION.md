...
> **v5.0 注記（FEPの位置づけの正直化）**:
> FEPは数学的枠組み（変分推論による自由エネルギー最小化）であり、
> 本実装は「FEPに**触発された**」設計である。FEPの用語をそのまま使うのは
> 以下の1コンポーネントのみ。
>
> - **サプライズ層（v5.0, 数学的に本物）**: 世界モデルの予測（μ, σ）と
>   実際の評価値 x から、負の対数尤度のガウス近似
>   `S = (x−μ)²/σ² + ln σ` を計算し、駆動の新奇性・学習率・行動選択に反映。
>   「環境変化でサプライズがスパイクし、安定で減衰する」ことをテストで検証。
>
> - **駆動層のホメオスタシス・学習層の予測誤差**: FEPの骨格（予測誤差→修正）
>   に相当する実装だが、確率分布・変分推論・KLは用いない（inspired-by）。
>
> 残りの層（人格・計画・長期計画など）はFEPの比喩的な設計指針としてのみ
> 使用しており、FEPを「適用した」とは主張しない。
