...
`main.py` 起動時にバックグラウンドスレッドとして自動起動し、5 分おきに状態更新・30 分おきに記憶統合を行います。

```mermaid
flowchart TD
    START["自律ループ起動"] --> TICK["_tick()（5分ごと）"]
    TICK --> DRV["tick_drives()<br/>経過時間で駆動値を更新<br/>欲求は育ち・親密度は薄れる"]
    DRV --> CONV{"30分経過?"}
    ...
    ACT -- "全駆動が低い" --> N
    R --> SAVE["Episode 保存<br/>on_autonomous_action() relief"]
    X --> SAVE
    C --> SAVE
    N --> SLEEP["300秒待機"]
    SAVE --> SLEEP
...
| 駆動値 | 時間経過 | 対話後 | 役割 |
|---|---|---|---|
| `curiosity` 好奇心 | 育つ | やや解消 | 探索・学習 |
| `connection` 親密度 | 薄れる | 上昇 | **本性解放のゲージ** |
| `creation` 創作欲 | 育つ | 変化なし | 創作（詩・曲） |
| `loneliness` 孤独 | 育つ | 大きく解消 | 不在感の演出 |
| `boredom` 退屈 | 育つ | 解消 | 単調ループ突破の動機 |

行動は**ジッタ付き主駆動選択**（拮抗時はランダム）で決まり、毎回同じ行動に収束しません。実行した行動は「満たされた欲求（relief）」を減らします（ホメオスタシス）。
...
5 分おきに駆動値を更新しながら自ら行動（内省/探索/創作/待機）し、30 分おきに記憶を統合します。
ループ間隔は環境変数 `LUCINA_LOOP_INTERVAL`（秒）で変更できます（例: `LUCINA_LOOP_INTERVAL=60`）。
...
| モジュール | 責務 |
|---|---|
| `character.py` | モニカの**不変核**。シード記憶（`seed`）、自己モデル（表/本性の2層）、話し方パターン、関係性、**駆動値ダイナミクス**（時間で更新・対話で更新・relief）と **モード導出**（建前/本性） |
...
| `loop.py` | 自律ループ。駆動値から行動決定（ホメオスタシス）、定期統合。main.py からバックグラウンド起動 |
